...
from colander_data_converter.base.models import Event, EventType, ObservableType, Observable, Artifact, DetectionRule, Device, \
    ArtifactType, DetectionRuleType, DeviceType
...
def test_creates_event_with_all_fields():
    ...
    artifact = Artifact(name='file.txt', type=ArtifactType(id=uuid4(), short_name='TXT', name='Text File'))
    detection_rule = DetectionRule(name='Rule', type=DetectionRuleType(id=uuid4(), short_name='RULE', name='RuleType'),
                                   content='rule')
    device = Device(name='Device', type=DeviceType(id=uuid4(), short_name='DEV', name='DeviceType'))
